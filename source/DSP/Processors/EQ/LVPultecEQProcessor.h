...
#include "LVPultecEQProcessBlock.h"

namespace viator
{
    class LVPultecEQProcessor
            : public viator::BaseProcessor, public juce::AudioProcessorValueTreeState::Listener {
    public:
        //==============================================================================
        explicit LVPultecEQProcessor(int id);

        ~LVPultecEQProcessor() override;
    ...
    private:
        ...
        std::unique_ptr<LVPultecEQParameters::parameters> m_parameters;

        void updateParameters();

        std::array<viator::LVPultecEQProcessBlock, 5> m_process_blocks;

        std::array<juce::SmoothedValue<float>, 2> m_mutes;
        juce::AudioBuffer<float> m_dry_buffer;

        //==============================================================================
        JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(LVPultecEQProcessor)
    };
}
